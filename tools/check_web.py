...
# The same ruling's words, spoken by a toast instead of by an element
# the page ships (owner ruling 2026-08-23; #275 and #265 still rule the
# WORDS, and they are byte-identical to what RULED_LINES pinned).
#
# Each row is (script, constant, ruled text, the page the toast lands
# on). All four halves are checked, because any one of them alone fails
# open in a different direction:
#
#   the constant's text   - the ruling itself, the half that moved;
#   the constant EXISTS   - a pin over a name no file holds is a pin
#                           that cannot fail;
#   it reaches showToast  - a constant nobody hands to the toast is a
#                           sentence nobody reads, and the words would
#                           still be "in the file";
#   the page's #toast     - BinderUI.showToast() finds its element by
#                           that id and RETURNS SILENTLY when there is
#                           none, so a door with no #toast turns the
#                           acknowledgement into a no-op with every
#                           source-text check still green.
#
# Read off apps/web alone: dist/ is the build of apps/web and the gate
# has its own stage refusing a dist that is not, so a second read here
# would pin the same fact twice.
RULED_TOAST_LINES = (
    ("auth.js", "SIGNED_OUT_LINE", "Signed out.", "index.html"),
)
...
def ruled_toast_line_problems():
    ...
    for row in RULED_TOAST_LINES:
        ...
        if not os.path.exists(path):
            problems.append((script,
                "is pinned in RULED_TOAST_LINES in tools/check_web.py and "
                "is not a script in apps/web. The owner's ruled line \"%s\" "
                "lives there; restore the file, or move the pin with the "
                "words" % ruled))
            continue
...
def register_problems():
    ...
    problems.extend(ruled_toast_line_problems())
