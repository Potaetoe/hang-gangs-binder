 ...
 *
 * ITS REPLACEMENT (0.9-M3-S10, #416, RE-TRUED 0.9-M3-S13/#433 and
 * 0.9-M3-S30/#452): apps/web/admin.js calls GET /admin-log (S8's real,
 * landed route name - #414 completion, comment 5370945709: "admin"
 * cannot be an API segment without colliding with
 * apps/web/admin.html's own URL through html_handling, so the ticket's
 * own /admin/log spelling was corrected before it shipped). The
 * original comment here said S8's branch was still under review and
 * this exception would close once it landed - that branch landed long
 * ago and dev/demo-stub.js still answers nothing for it, so the
 * sentence describing a transitional state was false: this is a
 * STANDING gap, not a countdown. The demo (dev/demo-stub.js) models
 * the pre-0.9-M3 Worker surface and has not grown with the admin API
 * 0.9-M3 is adding; 0.9-M3-S13 (#433) added PUT/DELETE
 ...
 * its own /spec call - but 0.9-M3-S14 (#434, merged forward into this
 * branch after S30, so the two never saw each other in the same tree
 * until now) gave `apps/web/charts.js` an unrelated call to the SAME
 * route (the effective spec, read once at setup to build the filter
 * chips from - this file's own header). GET /spec joins the gap list
 * for that reason alone: nothing about the admin API, and the demo has
 * never modeled it for any caller. Building real stub answers for the
 * admin API is out of scope for a page-only slice; the honest stub
 * answer for all of it stays the same 404 an unknown path gets.
...
await check("the stub answers every call apps/web makes, by verb and " +
  ...
  "/admin-fields, PUT/DELETE /admin-fields/<id>) and GET /spec (the " +
  "demo has not grown with 0.9-M3's admin surface, nor with the " +
  "effective-spec route 0.9-M3-S14's charts page now reads)", () =>
