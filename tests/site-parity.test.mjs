 ...
 * and titles, form.js's factors, limits and choices, dashboard.js's
 * unit table, query.js's splits, and your-page.html's own markup.
 *
 ...
 *
 * THE ROSTER IS PINNED HERE rather than counted from whatever it finds.
 * Four pages carry the wordmark. Counting the copies it discovers
 * would make a page that silently lost its mark a page this arm
 * happily agrees with - the failure that pin exists to catch is
 * subtraction, and a count derived from the tree cannot see it.
 *
