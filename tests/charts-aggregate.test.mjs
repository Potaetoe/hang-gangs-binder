...
check("bins: merging widens a bin rather than adding one - the drawn " +
  ...
  tail.distribution.bins.every((b, i, all) =>
    i === 0 || b.from[tail.distribution.partition.system] ===
      all[i - 1].to[tail.distribution.partition.system]));
...
check("Other: a categorical measure draws cells rather than bins",
  byCountry.enough === true && byCountry.distribution.kind === "cells");
check("Other: every drawn cell clears the floor",
  byCountry.distribution.cells.every((c) => c.count === 0 || c.count >= FLOOR));
check("Other: the fold lands in one bucket that itself clears the floor",
  (() => {
    const other = byCountry.distribution.cells
      .filter((c) => c.bucket === "other");
    return other.length === 1 && other[0].count >= FLOOR;
  })());
check("Other: the drawn cells sum to the population, so the remainder " +
  "cannot be differenced back",
  byCountry.distribution.cells.reduce((n, c) => n + c.count, 0) ===
    spread.length);
check("Other: the bucket's label carries no member-typed value " +
  "(mandate 5)",
  byCountry.distribution.cells.filter((c) => c.bucket === "other")
...
const partitioned = agg.aggregate(evenGroup(FLOOR + 8), ask("measure=weight"));
const systems = Object.keys(partitioned.units);

check("one partition: every unit system reports the same number of bins",
  systems.length > 1 &&
  systems.every((s) => partitioned.distribution.bins.every((b) =>
...
check("one partition: the counts are one set of numbers, not one per " +
  ...
  partitioned.distribution.bins.every((b) => typeof b.count === "number"));
...
for (let trial = 0; trial < 200; trial += 1) {
  const rows = [];
  const people = FLOOR + Math.floor(Math.random() * 20);
  for (let i = 0; i < people; i += 1) {
  ...
  const bins = answer.distribution.bins;
  const metric = bins.map((b) => [b.from.metric, b.to.metric]);
  const imperial = bins.map((b) => [b.from.imperial, b.to.imperial]);
  checked += 1;
...
check("route: no account id, handle or row id appears anywhere in a " +
  ...
  !everyString(drawn.body).includes("telegram"));
