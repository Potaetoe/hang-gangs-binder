...
| A1.2 | Load the sign-in page again at both frame widths — **Desktop**, then **Phone** | The cover opens once per load and never traps the page behind it | Its resting state is *open*, so every way the animation can fail leaves the sign-in reachable. While you have both widths in front of you, **note the cover's shape at each** — whether it fills the frame edge to edge, or stands as a centered column with page background beside it — and write down what you saw: the shape differs between the two widths and nobody has ruled on whether that is what the entrance should be |
| A1.3 | Turn on the operating system's reduced-motion setting and reload | The binder is **already open** — no frame of the closed cover at all | At a shortened duration the keyframes still run, and the rule that shortens them leaves the delay alone — so what a reader would be given is not a flash but the closed cover held opaque for about a fifth of a second, over the whole frame at the phone width and over the sign-in column at the desktop one. That is the exact thing that setting asks not to be given |
...
| A3.9 | Read the code under **How your entry is handled**, which the page asks you to compare against the one pinned in the group chat | 32 characters, matching the **development** public key in `config.js` | On anything but production it will not match the pinned group message, and that is expected |
| A3.10 | Under **On record**, find **Your entries, opened** and read what it says about the rows it could not open | Every row this browser cannot open is **counted and named** — "*n* were sealed on another device, so they are not in the answer above" — and the answer above it is not quietly computed over the rest. When **nothing** opened, that line is absent and a single sentence says so instead (#265 row 17: both printed together before it, the second claiming rows were missing from an answer that was never drawn). #85's personal pane. **The demo's rows are placeholder bytes sealed to nobody by design**, so what this arm accepts is that the pane reports its own blindness honestly; a pane that drew a confident answer over rows it never opened would pass a check written any other way | An answer computed over fewer rows than the member has is an answer they cannot tell from a correct one. Staged, this is the only half that can be driven — AL7 is the round trip |
...
> standing, two entries resting behind them — so every row below is read
...
| # | Do | Pass looks like | Why |
| --- | --- | --- | --- |
| A5.1 | Read **On record**, which the stop opens on | Two numbers, not one: the entries standing, and a separate line naming the entries resting behind them | The whole of #193. Six rows written and four claimed is what a member who corrected twice has, and one number cannot say it |
